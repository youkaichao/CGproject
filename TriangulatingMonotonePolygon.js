...
function TriangulatingMonotonePolygon(points) {
    ...
    function chopOffTriangle(s, t, c) {
        output.push([s, t, c]);
        events.push(generateEvent(output, "insert"));
    }
    ...
    for (let i=2; i<points.length; i++) {
        let c = points[i], t = stack[stack.length-1], s = stack[stack.length-2];
        if (c.position === t.position) { // Case A: c lies on the same chain as t
            if (isReflex(s, t, c)) { // t is reflex
                stack.push(c);
            } else { // t is convex
                while (true) {
                    chopOffTriangle(stack[stack.length-2], stack[stack.length-1], c);
                    stack.pop();
                    if (stack.length === 1 || isReflex(stack[stack.length-2], stack[stack.length-1], c)) {
                        break;
                    }
                }
                stack.push(c);
